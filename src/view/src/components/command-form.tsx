...
export const CommandForm = ({ command, commands, formId, onSave }: CommandFormProps) => {
  const schema = useMemo(
    () => createCommandFormSchema(commands, command?.index),
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    if (isGroupMode) {
      commandConfig.group = data.group;
      commandConfig.executeAll = data.executeAll;
    } else {
      commandConfig.command = data.command;
      commandConfig.useVsCodeApi = data.useVsCodeApi;
      commandConfig.terminalName = data.terminalName || undefined;
    }
  ...
  const handleConfirmConversion = () => {
    if (pendingSave) {
      const commandConfig: ButtonConfig = {
        color: pendingSave.color || undefined,
        command: pendingSave.command,
        id: pendingSave.id,
        name: pendingSave.name.trim(),
        shortcut: pendingSave.shortcut || undefined,
        terminalName: pendingSave.terminalName || undefined,
        useVsCodeApi: pendingSave.useVsCodeApi,
      };

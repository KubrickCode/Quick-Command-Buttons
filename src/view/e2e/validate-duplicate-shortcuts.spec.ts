...
test.describe("Test 12: Validate Duplicate Shortcuts", () => {
  test("should show validation error when entering duplicate shortcut and block save", async ({
    ...
    await expect(
      page.getByText(`Shortcut "${NEW_COMMAND.duplicateShortcut}" is already used by another command`)
    ).toBeVisible();
    ...
    const initialCommandCount = 3; // Test, Terminal, Git
  ...
  test("should allow same shortcut when editing the same command", async ({ page }) => {
    ...
    await expect(
      page.getByText(`Shortcut "${EXISTING_COMMAND.shortcut}" is already used by another command`)
    ).not.toBeVisible();
